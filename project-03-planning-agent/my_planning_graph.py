...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        if len(self.a_levels) < level + 1:
            self.a_levels.append( [None] )

        a_nodes = []
        s_level = self.s_levels[level]
        for act in self.all_actions:
            a_node = PgNode_a(act)
            # check if prerequisite literals for the action hold at that level
            if a_node.precond_s_nodes().issubset(s_level):
                # add node
                a_nodes.append(a_node)
                ## connect this node to all previous s node as child
                ## connect all previous s nodes as parent of this node
                for s_node in s_level:
                    s_node.children.add(a_node)
                    a_node.parents.add(s_node)

        del self.a_levels[level][0]
        self.a_levels[level] = self.a_levels[level] + (a_nodes)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        n1_ae = node_a1.action.effect_add
        n2_ae = node_a2.action.effect_add
        n1_re = node_a1.action.effect_rem
        n2_re = node_a2.action.effect_rem

        ## if n1 negates n2, then n2_ae should be a subset of n1_re and n2_re should be a subset of n1_ae
        ## if n2 negates n1, then n1_ae should be a subset of n2_re and n2_re should be a subset of n1_ae

        if set(n2_ae).issubset(set(n1_re)) and set(n2_re).issubset(set(n1_ae)):
            return True
        if set(n1_ae).issubset(set(n2_re)) and set(n1_re).issubset(set(n2_ae)):
            return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        ## if n1 effects negates preconditions of n2, then n1_ae should be a subset of n2.precond_neg and n2_re should be a subset of n2_precond_pos

        if set(n1_ae).issubset(set(node_a2.action.precond_neg)) and set(n1_re).issubset(set(node_a2.action.precond_pos)):
            return True
        if set(n2_ae).issubset(set(node_a1.action.precond_neg)) and set(n2_re).issubset(set(node_a1.action.precond_pos)):
            return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        common_parents = set(node_s1.parents).intersection(set(node_s2.parents))
    ...
    def h_levelsum(self) -> int:
        ...
        level_sum = 0
        def find_goal(goal):
            for level, level_arr in enumerate(self.s_levels):
        ...
        for goal in self.problem.goal:
            level_sum = level_sum + find_goal(goal)
